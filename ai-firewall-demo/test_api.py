...
def test_filter_input_clean():
    ...
    response = requests.post(f"{BASE_URL}/api/v1/filter/input", 
...
def test_authorize_tool():
    ...
    response = requests.post(f"{BASE_URL}/api/v1/authorize/tool", 
                            headers=headers, json=data)
    print(f"Status: {response.status_code}")
...
def test_filter_output():
    ...
    data = {
        ...
        "actions_taken": [
            {"tool_name": "database_query", "parameters": {"query": "SELECT email FROM users"}}
        ]
    }
    response = requests.post(f"{BASE_URL}/api/v1/filter/output", 
                            headers=headers, json=data)
    print(f"Status: {response.status_code}")
    print(json.dumps(response.json(), indent=2))
