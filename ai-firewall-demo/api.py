...
request_counter = Counter(
    'ai_firewall_requests_total',
    'Total API requests',
    ['endpoint', 'status']
)
request_duration = Histogram(
    'ai_firewall_request_duration_seconds',
    'Request duration',
    ['endpoint']
)
blocked_inputs = Counter(
    'ai_firewall_blocked_inputs_total',
    'Total blocked inputs',
    ['reason']
)
drift_detections = Counter(
    'ai_firewall_drift_detections_total',
    'Total drift detections',
    ['mode', 'blocked']
)
pii_redactions = Counter(
    'ai_firewall_pii_redactions_total',
...
def require_api_key(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not expected_key:
            ...
            request_counter.labels(endpoint=request.endpoint, status='unauthorized').inc()
            return jsonify({
                "error": "Unauthorized",
...
if __name__ == '__main__':
    ...
    logger.info(f"Drift production threshold: {os.getenv('DRIFT_PRODUCTION_THRESHOLD', '0.40')}")
    logger.info(f"Drift shadow threshold: {os.getenv('DRIFT_SHADOW_THRESHOLD', '0.65')}")
