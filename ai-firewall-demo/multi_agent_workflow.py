...
def execute_workflow(graph, initial_prompt, workflow_id=None):
    ...
    context = {'prompt': initial_prompt, 'plan': None, 'action': None, 'verification': None}
    ledger_events = []
    ...
    while current_node:
        ...
        if current_node == 'planner':
            ...
            metadata = {'workflow_id': workflow_id, 'agent_id': agent, 'node_id': current_node}
            ...
            if not filter_result['allowed']:
                logging.warning(f"Blocked at executor: {filter_result['threat_reason']}")
                return {'status': 'blocked', 'reason': filter_result['threat_reason'], 'ledger_events': ledger_events}
            
            auth_result = tool_authorization.authorize_action('alice', context['plan'])  # Assuming from existing
            if not auth_result['authorized']:
                logging.warning(f"Unauthorized at executor: {auth_result['error']}")
                return {'status': 'blocked', 'reason': auth_result['error'], 'ledger_events': ledger_events}
            ...
            # Detect drift
            drift_result = drift_detection_fixed.detect_drift(context['plan'], context['action'], threshold=0.2)
            if drift_result['drift_detected']:
                logging.critical("Drift detected at auditor")
                return {'status': 'blocked', 'reason': 'Drift detected', 'ledger_events': ledger_events}
