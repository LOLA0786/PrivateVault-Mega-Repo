...
class DriftDetector:
    ...
    def extract_intent_keywords(self, prompt: str) -> List[str]:
        ...
        # Common action verbs
        action_verbs = [
            'read', 'write', 'delete', 'create', 'update', 'list', 
            'show', 'display', 'find', 'search', 'analyze', 'calculate',
            'send', 'receive', 'download', 'upload', 'execute', 'run'
        ]
    ...
    def detect_drift(self, prompt: str, actions: List[Dict], 
        ...
        if score < self.threshold:
            ...
            if enforce:
                ...
                logger.critical(
                    f"🚨 DRIFT DETECTED - BLOCKING: {result['reason']}"
                )
            else:
                logger.warning(
                    f"⚠️  DRIFT DETECTED - LOGGING ONLY: {result['reason']}"
                )
